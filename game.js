  ...
  function render(){
    ...
    function getPlayerName(playerSymbol){
      if(playerSymbol === state.players[0].symbol)
        return state.players[0].isComputer ? 'Computadora' : "Jugador1"
      return state.players[1].isComputer ? 'Computadora' : "Jugador2"
    }
    ...
    function htmlQ1(){
      return `<div id="view1"><h3>Que modo de juego deseas?\n</h3>
      ${buttonHTML(1, "1player", "Jugador Vs Computadora")}
      ${buttonHTML(1, "2players", "Jugador Vs Jugador")}
      </div>`
    }

    function htmlQ2(){
      const html2=`<div id="view2"><h3>${!state.players[1].isComputer? "Jugador 1, <br />" : ""}Que simbolo deseas usar?</h3>
      ${buttonHTML(2, "X", "X")}
      ${buttonHTML(2, "O", "O")}`
      return html2
    }
    ...
    function htmlGame (){
      ...
      const playerName = state.game.turn === 0 ? 'Jugador1' : state.players[1].isComputer ? 'Computadora' : 'Jugador2'
    //   let playerName = 'Computer'
    //   if(!state.players[state.game.turn].isComputer)
    //     playerName = state.game.turn === 0 ? 'Player1' : 'Player2'

      let htmlBefore = `<h3>Movimientos: ${moveNumber} ${htmlSpaces(5)} Turno: ${playerName}</h3>`
      let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
        ...
        let htmlAfter = `<h4>Puntuacion: ${htmlSpaces(1)} Jugador 1 - ${state.players[0].score} ${htmlSpaces(2)} ${state.players[1].isComputer? "Computadora" : "Jugador 2" } - ${state.players[1].score}</h4>`
    ...
    function htmlGameEnd (){
      ...
      if(result !== RESULT.tie)
        resultText = getPlayerName(result) + " Gano!"

      let htmlBefore = `<h3>${resultText} ${htmlSpaces(2)} Haz click para reiniciar</h3> `
      let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
